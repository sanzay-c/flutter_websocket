...
void main() {
  ...
  group('MessageBubble Widget Tests', () {
    testWidgets('renders user message correctly (blue bubble, right aligned)', (
      WidgetTester tester,
    ) async {
      // Arrange
      ...
      ); // Sent by me should be right aligned
    });

    testWidgets(
      'renders received message correctly (grey bubble, left aligned)',
      (WidgetTester tester) async {
        ...
        ); // Received should be grey

        final Row row = tester.widget(find.byType(Row).first);
        expect(
          row.mainAxisAlignment,
          MainAxisAlignment.start,
        ); // Received should be left aligned
      },
    );
  });
}
