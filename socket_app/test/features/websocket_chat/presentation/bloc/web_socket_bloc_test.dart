...
void main() {
  ...
  group('ConnectEvent', () {
    ...
    final tConnectionStatusStreamController =
    ...
    blocTest<WebSocketBloc, WebSocketState>(
      ...
      build: () {
        when(
          () => mockConnectWebSocket(),
        ).thenAnswer((_) async => Right(tMessageStreamController.stream));
        when(
          () => mockWebSocketRepository.connectionStatusStream,
    ...
    blocTest<WebSocketBloc, WebSocketState>(
      ...
      build: () {
        when(() => mockConnectWebSocket()).thenAnswer(
          (_) async => const Left(ServerFailure('Connection Failed')),
        );
        return bloc;
      },
      act: (bloc) => bloc.add(ConnectEvent()),
  ...
  group('DisconnectEvent', () {
    blocTest<WebSocketBloc, WebSocketState>(
      ...
      expect: () => [const WebSocketDisconnected()],
      verify: (_) {
  ...
  group('Security and Data Privacy', () {
    blocTest<WebSocketBloc, WebSocketState>(
      ...
      build: () {
        when(() => mockConnectWebSocket()).thenAnswer(
          (_) async =>
              const Left(ServerFailure('Unauthorized access - Token Expired')),
        );
        return bloc;
      },
      act: (bloc) => bloc.add(ConnectEvent()),
    ...
    blocTest<WebSocketBloc, WebSocketState>(
      ...
      expect: () => [const WebSocketConnected(messages: [])],
    );

    test(
      'bloc.close() should cancel all subscriptions (Memory Leak Prevention)',
      () async {
        // Arrange
        when(() => mockConnectWebSocket()).thenAnswer(
          (_) async => Right(StreamController<MessageEntity>().stream),
        );
        when(
          () => mockWebSocketRepository.connectionStatusStream,
